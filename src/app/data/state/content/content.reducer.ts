...
export const contentAdapter: EntityAdapter<Content> = createEntityAdapter<
  Content
>({
...
function filterDisplayContent(state: ContentState, action): ContentState {
  ...
  switch (FilterTypeMapping[filterMode]) {
    case FilterType.Genre:
      filterFunction = (allContents: Array<Content>, filterValue) => {
        return allContents.filter((content) =>
          checkList(content.genres, filterTerm)
        );
      };
      break;
    case FilterType.Language:
      filterFunction = (allContents: Array<Content>, filterValue) => {
        return allContents.filter(
          (content) =>
            content.language.toUpperCase() === filterValue.toUpperCase()
        );
      };
      break;
    case FilterType.None:
      filterFunction = (allContents, filterValue) => allContents;
  }
...
function sortDisplayContent(state: ContentState, action): ContentState {
  ...
  switch (SortOrderMap[sortOrder]) {
    case SortOrder.ASCENDING:
      sortedDisplayContents = sortByAscending(
        contentsToSort,
        sortByField,
        sortByFieldDataType
      );
    ...
    case SortOrder.DESCENDING:
      sortedDisplayContents = sortBydescending(
        contentsToSort,
        sortByField,
        sortByFieldDataType
      );
...
function sortByAscending(
  ...
  if (dataType == 'date') return allContents.sort(dateCompareFunction);
